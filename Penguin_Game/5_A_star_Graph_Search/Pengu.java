...
public class Pengu {
    ...
    static class Path{
     ...
     public static void main(String[] args) {
        ...
        if (penguPresentPosition != null) {
            ...
            AStar(box, penguPresentPosition.rows, penguPresentPosition.cols, output_Name);
        } else {
    ...
    // Function to perform the A* traversal
    static void AStar(String initialBox[][], int rows, int cols, String output_Name) {
        int score = 0;
        int finalGoal = 17;
    ...
    public static class PriorityQueueComparator implements Comparator<Path> {
        public int compare(Path path1, Path path2){
             if (path1.score < path2.score) {
               ...
               return cost(path1, path2);
             }
        }
    }

    // cost function
    public static int cost(Path path1, Path path2){
    if ((path1 != null) && (path2 != null)) {
        if (path1.path.length()  < path2.path.length()) {
            return -1;
        }else if (path1.path.length() > path2.path.length()) {
            return 1;
        } else { 

 ...
 *
 */
class MoveBoard{ //This class is created to move and its corresponding executed chessboard.
	public ChessBoard chessboard=new ChessBoard();
	public ChessBoard mainChessBoard=new ChessBoard();//We are storing the original board for which we have generated the valid moves and its corresponding boards. 
	public String move="";
	//The comparator function helps to order the array list of moves and its corresponding executed board
	public static Comparator<MoveBoard> BoardComparator = new Comparator<MoveBoard>() {
		public int compare(MoveBoard s1, MoveBoard s2) {
	    	return s1.chessboard.getMoveValue(s1.move,s1.mainChessBoard)-s2.chessboard.getMoveValue(s2.move,s2.mainChessBoard);
	    }
	};
}
...
public class ChessBoard {
	...
	String executedMove="";
	int whitePawanPromotionCount=0;
	int blackPawanPromotionCount=0;
	int whiteKingCheckCount=0;
	int blackKingCheckCount=0;
	int blackKillWhiteCount=0;
	int whiteKillBlackCount=0;
	...
	public boolean verifyKingCheckStatus(String location) {//Here we are checking whether the king is in check status or not
		...
		for (int i = 0; i < oppositeTeamValidMoves.size(); i++) {//Here we are checking all the opponent valid moves and each opponent valid move has source and destination
			...
			if (location.equals(tempmove)) {//If both are equal then we are returning true as check status. 
				...
				if(Character.isUpperCase(oppositeTeamValidMoves.get(i).charAt(2))) {
					this.blackKingCheckCount++;
				}
				else {
					this.whiteKingCheckCount++;
				}
				return true;
			}
		}
	...
	public void executeMove(String move) {//This executeMove function takes input move as a String and executes the move
	...
	public ArrayList<String>  filterValidMovesConsiderKingCheckConditions() {//The method helps to select only the moves which help to remove or nullify the existing check. If there is no existing check it will return all validmoves. 
		...
		return validMoves;//returning the random move generated from the filtered list. 
	}
	
	
	public int chessboardPiecesCount() {//This method helps to give the total number of chess pieces present on the board including black and white
		int count=0;
		for(int i=0;i<8;i++) {
			for(int j=0;j<8;j++) {
	...
	public int getMoveValue(String move,ChessBoard chessboard) {//This method generates the value of each move, based on this value we are trying to implement the move order based on move values
		//The comparator uses this method to compare two moves and order the array list of moves and its corresponding boards.
		String[][] promotionPieceValue= {{"r","35"},{"n","20"},{"b","25"},{"q","50"}};
		String[][] killerPieceValue= {{"p","60"},{"n","50"},{"b","45"},{"r","30"},{"q","20"},{"k","10"}};
		int moveValue=0;
		if(move.charAt(0)=='0') {
			ChessBoard tempChessBoard=new ChessBoard();
			tempChessBoard=chessboard.copyChessBoardObject();
			tempChessBoard.executeMove(move);
			if(chessboard.chessboardPiecesCount()>tempChessBoard.chessboardPiecesCount()) {
				for(int i=0;i<killerPieceValue.length;i++) {
					if(String.valueOf(move.charAt(2)).equals(killerPieceValue[i][0])) {
						moveValue=5*Integer.parseInt(killerPieceValue[i][1]);
					}
				}
			}	
		}
			...
			for(int i=0;i<promotionPieceValue.length;i++) {
				if(String.valueOf(move.charAt(2)).equals(promotionPieceValue[i][0])) {
					moveValue=5*Integer.parseInt(promotionPieceValue[i][1]);
				}
			}
		}
	...
	public void changePlayerSide() {//This method helps to change the Player's side, if it is white then it will change to black and vice versa.
	...
	public ArrayList<MoveBoard> generateMoveAndItsCorrespondingBoards(ChessBoard chessboard){//The generateMoveAndItsCorrespondingBoards takes input as ChessBoard object and generates valid moves by considering King check conditions
		//and each valid moves is executed on the chessboard. Each valid move and its corresponding executed chessboard is store in the MoveBoard class object.
		ArrayList<MoveBoard> moveAndBoards=new ArrayList<MoveBoard>();
		ChessBoard tempChessBoard=new ChessBoard();
		ArrayList<String> validMoves = new ArrayList<String>();
		
		tempChessBoard=chessboard.copyChessBoardObject();
		tempChessBoard.generateValidMoves();
		validMoves=tempChessBoard.filterValidMovesConsiderKingCheckConditions();
		for(int i=0;i<validMoves.size();i++) {
			ChessBoard tempChessBoard1=new ChessBoard();
			MoveBoard moveboard=new MoveBoard();
			tempChessBoard1=tempChessBoard.copyChessBoardObject();
			moveboard.move=validMoves.get(i);
			moveboard.mainChessBoard=chessboard;
		    tempChessBoard1.executeMove(validMoves.get(i));
			moveboard.chessboard=tempChessBoard1.copyChessBoardObject();
			tempChessBoard1=chessboard.copyChessBoardObject();
			moveboard.chessboard.executedMove=moveboard.move;
			moveAndBoards.add(moveboard);
		}
		return moveAndBoards;
	}
	 
	
	public ArrayList<String> generateCaptureMoves(ChessBoard chessboard){//This method tries to generate the capture moves from all the valid moves.
		ArrayList<MoveBoard> moveAndBoards=new ArrayList<MoveBoard>();
		ArrayList<String> validCaptureMoves = new ArrayList<String>();
		moveAndBoards=generateMoveAndItsCorrespondingBoards(chessboard);
		int initialPiecesCount=chessboard.chessboardPiecesCount();
		for(int i=0;i<moveAndBoards.size();i++) {
			ChessBoard tempChessBoard=new ChessBoard();
			tempChessBoard=moveAndBoards.get(i).chessboard;
			int finalPiecesCount=tempChessBoard.chessboardPiecesCount();
			if(initialPiecesCount>finalPiecesCount) {
				validCaptureMoves.add(moveAndBoards.get(i).move);
			}
		}
		return validCaptureMoves;
	}
	
	public ArrayList<MoveBoard> generateCaptureMovesandItsCorrespondingBoards(ChessBoard chessboard){//This method helps to generate all the capture moves from valid moves and its corresponding executed boards.
		ArrayList<MoveBoard> moveAndBoards=new ArrayList<MoveBoard>();
		ArrayList<String> validCaptureMoves = new ArrayList<String>();
		validCaptureMoves=generateCaptureMoves(chessboard);
		for(int i=0;i<validCaptureMoves.size();i++) {
			ChessBoard tempChessBoard=new ChessBoard();
			MoveBoard moveboard=new MoveBoard();
			tempChessBoard=chessboard.copyChessBoardObject();
			tempChessBoard.executeMove(validCaptureMoves.get(i));
			tempChessBoard.executedMove=validCaptureMoves.get(i);
			moveboard.chessboard=tempChessBoard;
			moveboard.mainChessBoard=chessboard;
			moveboard.move=tempChessBoard.executedMove;
			moveAndBoards.add(moveboard);
		}
		return moveAndBoards;
	}

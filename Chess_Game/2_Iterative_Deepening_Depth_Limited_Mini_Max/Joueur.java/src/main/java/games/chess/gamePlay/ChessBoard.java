...
public class ChessBoard {
	...
	public int HeuristicFunction() {//The Heuristic Function helps to calculate the Heuristic value of the chessboard.
		//In chessboard each piece is having a value. The sum of the values of all pieces is the heuristic value. 
		String s="pnbrqk";
		int n=0;
		int HeuristicValue= 0;
		int val[]= {1,3,4,5,9,100};
		for(int i=0;i<8;i++) {
			for(int j=0;j<8;j++) {
				if((this.sideToMove=='w')&&(Character.isUpperCase(this.gameBoard[i][j]))) {
					n=s.indexOf(String.valueOf(Character.toLowerCase(this.gameBoard[i][j])));
					HeuristicValue=HeuristicValue+val[n];
				}
				else if((this.sideToMove=='b')&&(Character.isLowerCase(this.gameBoard[i][j]))) {
					n=s.indexOf(String.valueOf(this.gameBoard[i][j]));
					HeuristicValue=HeuristicValue+val[n];
				}
			}
		}
